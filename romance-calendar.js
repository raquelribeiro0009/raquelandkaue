...
let currentTheme = 'bridgerton';
let events = JSON.parse(localStorage.getItem('romanceEvents')) || {};
let selectedEventDate = null;

// Temas das séries de romance
const themes = {
    bridgerton: {
        colors: ['#ffeef8', '#f8e8ff', '#ffe8f3'],
        accent: '#ff69b4',
        icon: '👑'
    },
    heartstopper: {
        colors: ['#e8f5ff', '#f0f8ff', '#e6f3ff'],
        accent: '#4ecdc4',
        icon: '💖'
    },
    'virgin-river': {
        colors: ['#f0fff0', '#e8f5e8', '#f5fffa'],
        accent: '#228b22',
        icon: '🌸'
    }
};
...
function addEvent() {
    ...
    localStorage.setItem('romanceEvents', JSON.stringify(events));
...
function showEventModal(dateKey) {
    ...
    modalBody.innerHTML = `
        <div style="text-align: center; margin-bottom: 20px;">
            ...
            <p style="color: #666; font-size: 1rem;">Tema: ${getThemeName(event.theme)}</p>
        </div>
    `;
...
// Adicionar efeitos especiais de romance
function addRomanceEffects() {
    // Efeito de partículas românticas
    setInterval(() => {
        createRomanceParticle();
    }, 1500);
    
    // Efeito de brilho nos ícones
    const icons = document.querySelectorAll('.romance-icon');
    icons.forEach((icon, index) => {
        setInterval(() => {
            icon.style.filter = 'drop-shadow(0 0 15px #ff69b4) brightness(1.3)';
            setTimeout(() => {
...
// Criar partícula romântica
function createRomanceParticle() {
    ...
    particle.style.cssText = `
        ...
        animation: romanceParticleFloat 5s ease-out forwards;
        opacity: 0.8;
    `;
...
style.textContent = `
    ...
    @keyframes romanceParticleFloat {
        0% {
